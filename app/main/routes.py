...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            flash(_('User %(username)s not found.', usernname=username))
            return redirect(url_for('main.index'))
...
def messages():
    ...
    db.session.commit()
    page = request.args.get('page', 1, type=int)
    messages = current_user.message_received.order_by(Message.timestamp.desc()).paginate(page, current_app.config['POSTS_PER_PAGE'], False)
...
def export_posts():
    if current_user.get_task_in_progress('export_posts'):
        ...
        #db.session.commit()
        flash(_('Export posts feature is not availabe on heroku'))
